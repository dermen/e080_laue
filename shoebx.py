...
# modeler_file is a .npy file created using save_modelers=True
modeler_file = sys.argv[1]
PAUSE = 0.05
n_bins = 20
...
M = np.load(modeler_file, allow_pickle=True)[()]
...
while FIG.loop_counter < num_spots: 
    ...
    data_top = data_trust >= data_thresh
    ysel = y[trusted][data_top]
    xsel = x[trusted][data_top]
    sigZ_val = np.std(vals[ data_top])
    #sigZ_val = Z[trusted].std()

    for ax in ax0,ax1,ax2:
        ax.clear()
    im = ax0.imshow(data, cmap=cmap)
    #ax0.plot( xsel, ysel, 'rx')
    vmin, vmax = im.get_clim()
    ax1.imshow(model, vmin=vmin, vmax=vmax, cmap=cmap)
    ...
    for AX in [ax0, ax1, ax2]:
        ...
        AX.set_xticklabels([])
        AX.set_yticklabels([])
    ax0.set_title("data", pad=0, fontsize=8)
    ax1.set_title("model", pad=0, fontsize=8)
    ax2.set_title("Z (sigZ=%.1f)" % sigZ_val, pad=0, fontsize=8)

    FIG.suptitle("spot %d" % i_h) #h,k,l = %d,%d,%d; d = %f" %(h,k,l,d))
    if a is not None:
        a.remove()
    if b is not None:
        b.remove()
